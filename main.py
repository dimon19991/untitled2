...
class Event(db.Model):
    tablename = 'event'
    event_name = db.Column(db.Integer, primary_key=True)
    event_date = db.Column(db.Date)

    place_name_fk = db.relationship("Place", secondary=association_table, back_populates="event_name_fk")


class Place(db.Model):
    tablename = 'place'
    place_name = db.Column(db.String(20), primary_key=True)
    place_adress = db.Column(db.String(30))

    event_name_fk = db.relationship("Event", secondary=association_table, back_populates="place_name_fk")
...
Dima = Event(event_name = 1,
            event_date = "1998-10-29")

home = Place(place_name = "home",
            place_adress = "home")

Dima.place_name_fk.append(home)

db.session.add_all([Dima, home])
